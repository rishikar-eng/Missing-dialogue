...
**Compute (AWS Fargate — DEPLOYED 2026-07-24):** heavy QC runs dispatch to **AWS Fargate** on-demand (scale-to-zero) instead of running in-process; the EC2 stays the always-on brain. Teams `run` → EC2 launches an ECS task (`backend/job_entry.py`, same container image) → it writes the report zip + `status.json` to **S3** → Teams `status` reads ECS state + S3 and returns a presigned download link. Toggle with `DQC_COMPUTE=fargate` in `.env` (unset = in-process fallback). Box auth: the dispatcher passes a short-lived access token so the task never rotates the refresh token. Full resource list + gotchas in memory `fargate-compute`; plan in `docs/fargate-plan.md`. (The older Lambda fan-out idea is `docs/lambda-serverless-plan.md` — kept as a later option for whole-show batches.)
